import { AbstractContract } from '@ts-messaging/registry';
import {
  Logger,
  LoggerChild,
  RawSchema,
...
export class ConfluentContract extends AbstractContract {
  ...
  async isCompatible(rawSchema: RawSchema): Promise<boolean> {
    ...
    if (response.status === 200) {
      this.logger.info(
        `Schema with type="${rawSchema.__type}" compatible="${response.body.is_compatible}" with contract ${this.name}.`
      );
      this.logger.debug(
        `Schema with type="${rawSchema.__type}" compatible="${response.body.is_compatible}" with contract ${this.name}.`,
  ...
  async register<T extends SchemaObject = SchemaObject>(rawSchema: RawSchema) {
    ...
    if (response.status === 200) {
      ...
      if (!schemaAndVersion) {
        throw this.logger.proxyError(
          new Error(
            `Could not find the newly registered schema with id=${response.body.id} in contract with name="${this.name}"`
          )
        );
      }

      this.logger.info(
        `Successfully registered new schema for contract ${this.name} with id=${response.body.id}`
      );
  ...
  async findByRawSchema<T extends SchemaObject = SchemaObject>(
    ...
    if (response.status === 200) {
      ...
      this.logger.info(
        `Loaded schema with id="${schema.__id}" and version="${version}" for contract with name="${this.name}" by raw schema.`
      );
      ...
      if (options?.autoRegister || this.registry.autoRegisterSchemas) {
        this.logger.info(
          `Could not find schemaID for contract with name="${this.name}", trying auto register...`
        );
        return this.register(rawSchema);
      }
      this.logger.warn(
        `Could not find schemaID for contract with name="${
          this.name
  ...
  async findByVersion<T extends SchemaObject = SchemaObject>(
    ...
    if (response.status === 200) {
      ...
      this.logger.info(
        `Loaded schema with id="${schema.__id}" and version="${version}" for contract with name="${this.name}".`
      );
      ...
      this.logger.info(
        `Could not find schema for contract name="${this.name}" with version="${version}"`
      );
      return null;
    } else {
      throw this.logger.proxyError(
  ...
  async findById<T extends SchemaObject = SchemaObject>(
    ...
    if (response.status === 200) {
      this.logger.debug(
        `Found ${response.body.length} contracts for schema with id="${schemaID}"`
      );
      const version =
